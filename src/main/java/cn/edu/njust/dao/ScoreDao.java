...
public class ScoreDao {
	...
	public Score getSelectedScore(String stuID, String session) throws SQLException {
        String sql = "select stuID, session, total_score, score_1, score_2, score_3, score_4, score_5, current_rank, period_rank from score_overview where stuID = '" + stuID + "' and session = '" + session + "'";
        Connection conn = DbUtil.getCon();
        Score score = new Score();
        try {
        	PreparedStatement pst = conn.prepareStatement(sql);
